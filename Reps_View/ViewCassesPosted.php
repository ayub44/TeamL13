...
<meta charset="utf-8">
    ...
    <title>View Cases | RapidReps</title>
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="bradcam_text">
                        <h3 style="text-align:center;">View Cases Posted</h3>
                        <p style="text-align:center;">Below Are The Cases That You Have Posted</p>
                    </div>
                </div>
            </div>
        </div>
...
<table id="myTable" class="sortable">
    <thead >
    <tr class="header">
                <th title="Sort Table by Case ID">CaseId</th>
                <th title="Sort Table by Client First Name">Client First Name</th>
                <th title="Sort Table by Client Last Name">Client Last Name</th> 
                <th title="Sort Table by Location">Location</th>
                <th title="Sort Table by Fee">Fee per hour</th>
                <th title="Sort Table by Describtion">Description</th>
                <th title="Sort Table by applied Solicitor First Name">Applied Solicitor First Name</th>
                <th title="Sort Table by applied Solicitor Last Name">Applied Solicitor Last Name</th>
                <th title="Grant the user access to the website">Give Solicitor Case</th> 
            </tr>
        </thead>
    <tbody>
        <?php
...
$results = $pdo->query("SELECT * FROM appliedcasestable", PDO::FETCH_ASSOC);
foreach( $results as $row) {
print("<tr class='item'>");
print("<td data-label='Case Id:'>".$row['CaseId']."</td>");
print("<td data-label='First Name:'>".$row['ClientFirstName']."</td>");
print("<td data-label='Last Name'>".$row['ClientLastName']."</td>");
print("<td data-label='Location'>".$row['Location']."</td>");
Print("<td data-label='Fee'>".$row['Fee']."</td>");
Print("<td data-label='Fee'>".$row['Describtion']."</td>");
Print("<td data-label='Fee'>".$row['AppliedSoliFirstName']."</td>");
Print("<td data-label='Fee'>".$row['AppliedSoliLastName']."</td>");
("<td data-label='Fee'>".$row['AppliedSoliEmailAddress']."</td>");

print("<td data-label='Date'><a href='GiveCase/Givesolicitorcase.php?
&CaseId=$row[CaseId]
...
&AppliedSoliFirstName=$row[AppliedSoliFirstName]
&AppliedSoliLastName=$row[AppliedSoliLastName]
&AppliedSoliEmailAddress=$row[AppliedSoliEmailAddress]'><span class='yes'>Assign case to solictor</span></a>
</td>");
