<?php 
  require_once('dbutils.php');
  session_start();
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Thank you for reporitng the issue</title>
    <link rel="stylesheet" href="dbtestStyleSheet.css">
</head>
<body>
<div class=thankyoucontent>
        <div class="wrapper-1">
            <div class="wrapper-2">
        <?php
     $pdo = connect();
     //checling if connected to dbutilsCreateCasesTable
    //  if ( $pdo) {
    //      print "<h1>Successfully connected!</h1>\n";
    //  } else {
    //      print "<h1>Failed to connect.</h1>\n";
    //  }     


    //  //executing table creation
    // createReportIssuesTable($pdo);
    // $table = 'reportIssueTable';
                
    // //Checking if table exists
    // if ( tableExists( $pdo, 'reportIssueTable')) {    
    //     print "<p>$table exists</p>\n";
    //     } else {
    //     print "<p>$table does not exist</p>\n";
    // }
         
    //   Collecting data from the form
    $CaseId      = $_POST['CaseId'];
      $AppliedSoliFirstName      = $_POST['AppliedSoliFirstName'];
      $AppliedSoliEmailAddress  =  $_POST['AppliedSoliEmailAddress'];
        
    //      //respodning to user via email straigt away
        $receiver = $AppliedSoliEmailAddress;
         $subject = "Assigned Case";
         $body = "Dear $AppliedSoliFirstName, you have been assigned $CaseId";
         $sender = "From:Ayubyusuf90@gmail.com"; 

        if(mail($receiver, $subject, $body, $sender)){
             print "<h1>Dear Admin, You have succesfully assigned Case ID: $CaseId to solicitor $AppliedSoliFirstName</h1>\n";
         }else{
             echo "<h1>Sorry, failed to assign case</h1>";
         }                           
        ?>
        

        <a href="../ViewCassesPosted.php"><button class="go-home">Go back to Assign More Cases</button></a>

        </div>
            
            <div class="footer-like">
                <p></p>
        </div>
    </div>
</body>
</html>


