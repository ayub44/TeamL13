...
<meta charset="utf-8">
    ...
    <title>Issues Reported By Unregistered Users | RapidReps</title>
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="bradcam_text">
                        <h3 style="text-align:center;">Quaries of unregistered users</h3>
                        <p style="text-align:center;">Below Are The quaries submitted by unregistered users.</p>
                    </div>
                </div>
            </div>
        </div>
...
<table id="myTable" class="sortable">
    <thead >
        <tr class="header">
            ...
            <th title="Sort Table by Message">Message</th>
            <th title="Sort Table by Date Submitted">Date Submitted</th>
...
$results = $pdo->query("SELECT * FROM contact_form_table", PDO::FETCH_ASSOC);
foreach( $results as $row) {
print("<tr class='item'>");
("<td data-label='Date'>".$row['FormID']."</td>");
print("<td data-label='Name'>".$row['fname']."</td>");
print("<td data-label='Name'>".$row['lname']."</td>");
print("<td data-label='EmailAdd'>".$row['EmailAdd']."</td>");
print("<td data-label='Date'>".$row['Message']."</td>");
print("<td data-label='Date'>".$row['DateSubmitted']."</td>");


print("<td data-label='Date'><a href='UnregisteredUsersIssues/RespondToIssue.php? 
&FormID=$row[FormID]
&fname=$row[fname]
&lname=$row[lname]
&EmailAdd=$row[EmailAdd]
&Message=$row[Message]
&DateSubmitted=$row[DateSubmitted]'><span class='yes' title='Respond to Issue'>&#10003;</span></a>
</td>");

print("<td data-label='Date'><a href='UnregisteredUsersIssues/DeleteIssue.php?
&FormID=$row[FormID]
&fname=$row[fname]
&lname=$row[lname]
&EmailAdd=$row[EmailAdd]
&Message=$row[Message]
&DateSubmitted=$row[DateSubmitted]'><span class='no' title='Delete Issue'>&#10007;</span></a>
</td>");  
print("</tr>");
};
}
