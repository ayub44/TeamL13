...
<meta charset="utf-8">
    ...
    <title>New Applications | RapidReps</title>
                                ...
                                <div class="main-menu  d-none d-lg-block">
                                    <nav>
                                        <ul id="navigation">
                                            ...
                                            <li><a  class="current" href="#">Manage Users <i class="fa fa-angle-down"> </i> </a>
                                                <ul class="submenu">
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="bradcam_text">
                        <h3 style="text-align:center;">New Applicants</h3>
                        <p style="text-align:center;">Respond To Users Who Have Submitted An Application To Join The Website.</p>
                    </div>
                </div>
            </div>
        </div>
...
<table id="myTable" class="sortable">
    <thead >
        <tr class="header">
            ...
            <th title="Sort Table by Role">Role</th>
            <th title="Sort Table by Status">Status</th>
            <th title="Sort Table by Date Signed Up">Date Signed Up</th>
            <th title="Grant the user access to the website">Admit User</th> 
            <th title="Refuse to allow User to join Website.">Reject User</th> 
        </tr>
        </thead>
    <tbody>
        <?php
...
print("<tr class='item'>");
("<td data-label='Date'>".$row['SignUpID']."</td>");
print("<td data-label='Name'>".$row['FirstName']."</td>");
print("<td data-label='Name'>".$row['LastName']."</td>");
print("<td data-label='EmailAdd'>".$row['EmailAddress']."</td>");
("<td data-label='Date'>".$row['Password']."</td>");
print("<td data-label='EmailAdd'>".$row['Role']."</td>");
print("<td data-label='Date'>".$row['Status']."</td>");
print("<td data-label='Date'>".$row['DateSignedUp']."</td>");


print("<td data-label='Date'><a href='NewApplicantFiles/AddNewApplicantToCurrentUsers.php? 
...
&Role=$row[Role]
&DateSignedUp=$row[DateSignedUp]'><span class='yes' title='Admit User'>&#10003;</span></a>
</td>");

print("<td data-label='Date'><a href='NewApplicantFiles/RejectNewApplication.php?
&SignUpID=$row[SignUpID]
&FirstName=$row[FirstName]
&LastName=$row[LastName]
&EmailAddress=$row[EmailAddress]
&Password=$row[Password]
&Role=$row[Role]
&DateSignedUp=$row[DateSignedUp]'><span class='no' title='Reject User'>&#10007;</span></a>
</td>");  
print("</tr>");
};
}
